            ...
            [].slice.call(document.querySelectorAll('[data-control="select2"], [data-kt-select2="true"]')).map((function(e) {
                var t = {
                    ...
                    matcher: function(params, data) {
                        ...
                        var terms = $.trim(params.term).split(" ");

                        // Check if each term is present in the data.text
                        for (var i = 0; i < terms.length; i++) {
                            var term = terms[i].toLowerCase();

                            // Check if there is a space in the search term
                            if (term.indexOf(" ") > -1) {
                                // If there is a space, split the term further and check each sub-term
                                var subTerms = term.split(" ");
                                for (var j = 0; j < subTerms.length; j++) {
                                    var subTerm = subTerms[j];
                                    if (data.text.toLowerCase().indexOf(subTerm) === -1) {
                                        // Return `null` if any sub-term should not be displayed
                                        return null;
                                    }
                                }
                            } else {
                                // If there is no space, check the whole term
                                if (data.text.toLowerCase().indexOf(term) === -1) {
                                    // Return `null` if any term should not be displayed
                                    return null;
                                }
                            }
                        }

                        // If all terms are present, add " (matched)" to the text
