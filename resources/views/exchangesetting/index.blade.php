        ...
        <div class="toolbar" id="kt_toolbar">
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                ...
                <div class="d-flex align-items-center py-1">
                    @if ($settings['isAdd'])
                        <a href="{{ route('exchange-settings.create') }}" class="btn btn-sm btn-primary">{{ $settings['buttons']['add'] }}</a>
                    @endif
                </div>
            </div>
        </div>
            ...
            <div id="kt_content_container" class="container-xxl">
                <div class="card mb-5 mb-xl-8">
                    <div class="card-header border-0 pt-5">
                        ...
                        <div class="col-md-12">
                            {{ Form::open(['route' => ['exchange-settings.index'],'method' => 'get']) }}
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2 right">
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="{{ route('exchange-settings.index') }}" class="btn btn-danger"> Clear </a><button type="submit" class="btn btn-primary">
                                ...
                                <tr>
                                    ...
                                    @endforeach
                                        <td class="text-end">
                                                ...
                                                <a class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" href="http://181.41.141.146/get-balance/1" data-bs-toggle="modal" data-bs-target="#myModal">
                                                    <i class="fas fa-file"></i>
                                                </a>

                                            <form action="{{ route('exchange-settings.destroy',$row->id) }}" method="POST">
                                                @if ($settings['isShow'])
                                                    <a href="{{ route('exchange-settings.show',$row->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                        <i class="fas fa-file"></i>
                                                    </a>
                                                @endif
                                                @if ($settings['isEdit'])
                                                    <a href="{{ route('exchange-settings.edit',$row->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
...
@section('pageScript')
    $(document).ready(function () {
        $('body').on('click', '[data-bs-toggle="modal"]', function () {
            var modalBody = $($(this).data("bs-target") + ' .modal-body');
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    modalBody.html('Error loading the page. Please try again.');
                }
            });
        });
    });
