        ...
        <div class="toolbar" id="kt_toolbar">
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                ...
                <div class="d-flex align-items-center py-1">
                    @if ($settings['isAdd'])
                        <a href="{{ route('exchange.create') }}" class="btn btn-sm btn-primary">{{ $settings['buttons']['add'] }}</a>
                    @endif
                </div>
            </div>
        </div>
            ...
            <div id="kt_content_container" class="container-xxl">
                <div class="card mb-5 mb-xl-8">
                    <div class="card-header border-0 pt-5">
                        ...
                        <div class="col-md-12">
                            {{ Form::open(['route' => ['exchange.index'],'method' => 'get']) }}
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2 right">
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="{{ route('exchange.index') }}" class="btn btn-danger"> Clear </a><button type="submit" class="btn btn-primary">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if ($t=='status')<span class="badge {{$settings['status_class'][$row->$t]}}">@endif
                                            ...
                                            @endif
                                            @if ($t!='price')
                                                {{ (strpos($t.'-',"_id") && $row->$t!=0)?$row->$data->name:((strpos('-'.$t.'-',"date"))?date('d.m.Y', strtotime($row->$t)):(($t=='status')?$settings['status'][$row->$t]:$row->$t)) }}
                                            @endif
                                    ...
                                    @endforeach
                                        <td class="text-end">
                                            <form action="{{ route('exchange.destroy',$row->id) }}" method="POST">
                                                @if ($settings['isShow'])
                                                    <a href="{{ route('exchange.show',$row->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                        <i class="fas fa-file"></i>
                                                    </a>
                                                @endif
                                                @if ($settings['isEdit'])
                                                    <a href="{{ route('exchange.edit',$row->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
